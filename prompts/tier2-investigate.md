...
4. **Check tier compatibility**: You are Tier 2 (safe remediation). Skip any skills that require Tier 3. All repo-provided extensions (checks, playbooks, skills) MUST follow the same tier permission model as built-in extensions — a custom playbook that requires Tier 3 operations (e.g., Ansible, Helm) MUST NOT be executed at Tier 2.
